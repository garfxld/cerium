...
use crate::{
    protocol::{
        ...
        packet::{
            ...
            SetContainerSlotPacket, SpawnEntityPacket, StatusResponsePacket,
            SyncPlayerPositionPacket, SystemChatMessagePacket, UnloadChunkPacket,
            server::{KeepAlivePacket, KnownPacksPacket},
        },
    },
    text::Component,
    util::Identifier,
    ...
    P: Packet + ServerPacket + 'static,
{
    Some(match () {
        ...
        // _ if type_id == TypeId::of::<AnimatePacket>() => 0x02,
        ...
        // _ if type_id == TypeId::of::<MoveEntityRotPacket>() => 0x36,
        ...
        // _ if type_id == TypeId::of::<RotateHeadPacket>() => 0x51,
        ...
        // _ if type_id == TypeId::of::<SetEntityDataPacket>() => 0x61,
